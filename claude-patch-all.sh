...
if (( __rc != 0 )); then
  if (( __rc == 3 )); then
    # Названный не-fatal исход, той же формой, что у стендов: «предмета нет
    # по объявлению» -- все carrier-гейты текста несут шаги, выключенные
    # реестром our-steps-off.txt. Причина берётся ИЗ ВЫВОДА ПРИБОРА, общая
    # фраза вместо его строки не подставляется; прогон продолжается.
    __ec_line="$(printf '%s\n' "$__ec_out" | grep -a -m1 'ПРЕДМЕТА НЕТ ПО ОБЪЯВЛЕНИЮ' || true)"
    echo "РАЗБОР ВКЛЕИВАЕМОГО КОДА: НЕ ИЗМЕРЕНО — предмета нет по объявлению (${__ec_line:-строка прибора не найдена})" >&2
  elif (( __rc == 2 )); then
...
if env -u CLAUDE_JUDGE_TOOLS_DIR -u CLAUDE_LAUNCH_AGENTS_DIR \
  ...
  case $__rc in
    ...
    1) echo "РАСКАТКА РАСХОДИТСЯ С КАНОНОМ: launchd и ядро исполняют не те" >&2
       ...
       exit 1 ;;
    # КОНСТРЕЙНТ: код 7 -- расхождение, для которого направление ДОКАЗАНО по
    # каждому расходящемуся файлу (дом равен ПРОШЛОМУ состоянию канона, правок
    # В ДОМЕ нет). Ручное решение здесь не нужно и вредно: прежняя редакция
    # отвечала на доказанное направление тем же кодом 1, что и на недоказуемое,
    # и гейт печатал «Направление НЕ следует из расхождения» ПОВЕРХ собственного
    # доказательства -- вердикт противоречил своему же замеру.
    # КОНСТРЕЙНТ: раскатка обязана ПЕРЕСВЕРЯТЬСЯ. Копия, не подтверждённая
    # повторной сверкой, -- заявление о записи, а не свидетель её результата;
    # остаточное расхождение после копии = отказ.
    # КОНСТРЕЙНТ: 9>&- на обоих вызовах -- дескриптор замка конвейера не
    # наследуется сверкой, иначе она видит свой замок занятым (#221).
    7) echo "РАСКАТКА ОТСТАЛА ОТ КАНОНА, НАПРАВЛЕНИЕ ДОКАЗАНО: выравниваю дом по канону" >&2
       if env -u CLAUDE_JUDGE_TOOLS_DIR -u CLAUDE_LAUNCH_AGENTS_DIR \
            bash "$(dirname "$0")/scripts/probes-sync.sh" --to-home 9>&-; then
         :
       else
         __rc_roll=$?
         case $__rc_roll in
           3) echo "АВТОРАСКАТКА НЕ НАЧАТА: замок сверки держит другой писатель (rc=3) -- дом НЕ выровнен" >&2 ;;
           1) echo "АВТОРАСКАТКА НЕПОЛНА: часть файлов не скопирована (rc=1) -- дом НЕ выровнен" >&2 ;;
           *) echo "АВТОРАСКАТКА ОТКАЗАЛА (rc=$__rc_roll) -- дом НЕ выровнен" >&2 ;;
         esac
         exit 1
       fi
       if env -u CLAUDE_JUDGE_TOOLS_DIR -u CLAUDE_LAUNCH_AGENTS_DIR \
            bash "$(dirname "$0")/scripts/probes-sync.sh" --diff 9>&-; then
         echo "Раскатка инструментов судьи: дом выровнен по канону — раскатка полная"
       else
         __rc_re=$?
         case $__rc_re in
           7) echo "ПОСЛЕ АВТОРАСКАТКИ ДОМ ВСЁ ЕЩЁ ОТСТАЁТ (rc=7): копия прошла, а байты не совпали" >&2 ;;
           1) echo "ПОСЛЕ АВТОРАСКАТКИ ДОМ РАСХОДИТСЯ БЕЗ ДОКАЗАННОГО НАПРАВЛЕНИЯ (rc=1): решение ручное" >&2 ;;
           *) echo "ПОВТОРНАЯ СВЕРКА ПОСЛЕ АВТОРАСКАТКИ ОТВЕТИЛА КОДОМ $__rc_re: вердикта нет" >&2 ;;
         esac
         exit 1
       fi
       ;;
...
bash "$(dirname "$0")/tools/env-handles-live-guard.sh" --label "реальное дерево" --homes "$__env_guard_homes" 9>&- || {
  ...
  case $__rc in
    ...
    3) echo "ГВАРД ЖИВОСТИ НА ДЕРЕВЕ: ЕСТЬ РУЧКА БЕЗ ЧИТАТЕЛЯ (имена выше)" >&2
       exit 3 ;;
    # CONSTRAINT: код 4 не останавливает сборку -- гвард не вправе требовать
    # от машины подогнать раскладку каталогов под разделение домов; ветка без
    # exit возвращает конвейеру rc=0 (последняя команда -- echo).
    4) echo "ГВАРД ЖИВОСТИ НА ДЕРЕВЕ: НЕ ИЗМЕРЕНО -- дерево разошлось с разделением домов (имена выше)" >&2
       ;;
    ...
    6) echo "ГВАРД ЖИВОСТИ НА ДЕРЕВЕ: ЧИТАТЕЛЬ ТОЛЬКО В СБОРКЕ -- расхождение сборки и исходника (имена выше)" >&2
       exit 6 ;;
    7) echo "ГВАРД ЖИВОСТИ НА ДЕРЕВЕ: ЛИШНЯЯ ДЕКЛАРАЦИЯ -- имя объявлено внешним через --external, а в настройках его нет (имена выше)" >&2
       exit 7 ;;
...
bash "$(dirname "$0")/tools/step7-window-teeth.sh" 9>&- || {
  __rc=$?
  case $__rc in
    2) echo "ЗУБЫ ОКНА ШАГА 7: НЕ ИЗМЕРЯЛИ -- прибор отказал (rc=2)" >&2
       exit 2 ;;
    # CONSTRAINT: код 3 -- предмета нет НА ЭТОЙ МАШИНЕ (образ 2.1.278.orig,
    # форк, node/python3/perl). Дельта дерева не есть поломка прибора и
    # сборку не останавливает; слитая с кодом 2, она была бы неотличима от
    # красного base.
    3) echo "ЗУБЫ ОКНА ШАГА 7: НЕ ИЗМЕРЕНО -- предмета нет на этой машине (rc=3)" >&2 ;;
...
if [[ "${CLAUDE_PATCH_SKIP_BENCH:-0}" == "1" ]]; then
  ...
  if (( __benchrc == 0 )); then
    ...
    rm -f "$BENCH_LOG"
  elif (( __benchrc == 3 )); then
    # Стенд назвал исход «предмета нет» кодом 3: патч не вставляет ни одного
    # блока проб (сняты либо выключены реестром). Это не отказ прибора и не
    # расхождение поведения -- мерить нечего, и прогон ЕДЕТ ДАЛЬШЕ. Причину
    # печатает сам стенд; строки стенда поднимаются наружу целиком, общая
    # фраза вместо них не подставляется.
    __bench_subject="$(grep -a -m1 '^probe-bench: ПРЕДМЕТА НЕТ' "$BENCH_LOG" || true)"
    echo "СТЕНД ЗОНДОВ: НЕ ИЗМЕРЕНО — предмета нет (${__bench_subject:-строка стенда не найдена})" >&2
    grep -a '^  ' "$BENCH_LOG" | sed 's/^/  /' >&2 || true
    rm -f "$BENCH_LOG"
  ...
  # которую пристинная копия видит на отраве. Вывод ребёнка печатается
  # ЦЕЛИКОМ: строки «НЕ ИЗМЕРЕНА» и сводка -- часть вердикта, не шум; код
  # снят ДО любых пайпов.
  __sc_out="$(bun "$BENCH" --self-check --binary "$BIN" 2>&1)" && __rc=0 || __rc=$?
  printf '%s\n' "$__sc_out"
  if (( __rc != 0 )); then
    if (( __rc == 3 )); then
      # Код 3 -- названный не-fatal исход: предмета нет, мутации, требующие
      # предмета, объявлены НЕ ИЗМЕРЕННЫМИ поимённо (выше), и прогон
      # продолжается. Строка стенда берется из его собственной сводки.
      __sc_line="$(printf '%s\n' "$__sc_out" | grep -a -m1 'SELF-CHECK.*предмета нет' || true)"
      echo "СТЕНД ЗОНДОВ: НЕ ИЗМЕРЕНО — предмета нет (${__sc_line:-строка стенда не найдена})" >&2
    else
      case $__rc in
