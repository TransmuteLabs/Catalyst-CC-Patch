...
# Разделение опирается на типизацию самого tweakcc: строка слоя промтов несёт
# префикс одного из семи типов (те же префиксы -- в именах файлов каталога),
# строка кода несёт вид «Имя -- описание» и двоеточия после типа не имеет.
# Если апстрим заведёт ВОСЬМОЙ тип, его строки уедут в счёт кода и двусторонняя
# дверь кода покраснеет -- отказ, а не тишина.
__TW_PROMPT_TYPES='Agent Prompt|Data|Skill|System Prompt|System Reminder|Tool Description|Tool Parameter'

# LC_ALL=C во всех трёх счётчиках: вывод содержит не-ASCII (✓/✗), а grep на
# такой строке под локалью машины считает мимо -- измерено на этом же ките.
__tw_applied_code_level() {   # <вывод tweakcc> -> число легших КОД-правок
  LC_ALL=C grep -a '^    ✓ ' "$1" 2>/dev/null \
    | LC_ALL=C grep -a -c -v -E "^    ✓ (${__TW_PROMPT_TYPES}): " || true
}

__tw_applied_prompt_level() { # <вывод tweakcc> -> число легших ПРОМТ-накладок
  LC_ALL=C grep -a -c -E "^    ✓ (${__TW_PROMPT_TYPES}): " "$1" 2>/dev/null || true
}
...
__tw_check_applied_level() {
  local __out="$1" __bin="$2"
  local __ver __code __prompts __nf __want_code __want_prompts
  ...
  if [[ "${CLAUDE_PATCH_ALLOW_TWEAKCC_FAILURES:-0}" == "1" ]]; then
    ...
    echo "NOTE: CLAUDE_PATCH_ALLOW_TWEAKCC_FAILURES=1 -- дверь уровня tweakcc погашена (кода $__code, промтов $__prompts)" >&2
    return 0
  fi
  ...
  __want_code="$(awk -F'\t' -v v="$__ver" '
      /^[[:space:]]*#/ { next } NF==0 { next }
      { k=$1; gsub(/^[[:space:]]+|[[:space:]]+$/,"",k) }
      k==v { gsub(/^[[:space:]]+|[[:space:]]+$/,"",$2); print $2; exit }
    ' "$TWEAKCC_EXPECTED_APPLIED" 2>/dev/null)"
  __want_prompts="$(awk -F'\t' -v v="$__ver" '
      /^[[:space:]]*#/ { next } NF==0 { next }
      { k=$1; gsub(/^[[:space:]]+|[[:space:]]+$/,"",k) }
      k==v { gsub(/^[[:space:]]+|[[:space:]]+$/,"",$3); print $3; exit }
    ' "$TWEAKCC_EXPECTED_APPLIED" 2>/dev/null)"
  if [[ -z "$__want_code" ]]; then
    echo "FATAL: для $__ver уровень tweakcc не объявлен, а измерено: кода $__code, промтов $__prompts." >&2
    ...
    if (( __rows == 0 )); then
      ...
      printf '%s\t%s\t%s\t<причина/происхождение чисел>\n' "$__ver" "$__code" "$__prompts" >&2
    else
      ...
      echo "  Вид строки: <версия><TAB><код><TAB><промты><TAB><причина>." >&2
      echo "  Измерено сейчас: кода $__code, промтов $__prompts." >&2
    fi
    return 1
  fi
  if [[ ! "$__want_code" =~ ^[0-9]+$ ]]; then
    ...
    echo "FATAL: в $TWEAKCC_EXPECTED_APPLIED уровень кода для $__ver не число: «${__want_code}»." >&2
    return 1
  fi
  # Слой КОДА -- двусторонне: его владелец один (версия апстрима + наш форк),
  # и просадка и прибавка одинаково означают, что объявление разошлось с делом.
  if (( __code < __want_code )); then
    echo "FATAL: слой кода tweakcc просел на $__ver: объявлено $__want_code, легло $__code." >&2
    echo "  Крестиков при этом может не быть ни одного: когда данных под версию" >&2
    echo "  нет, правки не пробуются, и сверка непроходов такую версию считает" >&2
    echo "  чистой. Разберитесь с причиной либо, если уровень изменился" >&2
    echo "  осознанно, поправьте строку в $TWEAKCC_EXPECTED_APPLIED" >&2
    return 1
  fi
  if (( __code > __want_code )); then
    echo "FATAL: на $__ver кода легло БОЛЬШЕ объявленного: объявлено $__want_code, легло $__code." >&2
    echo "  Объявление пережило свою причину -- правки снова ложатся, а запись" >&2
  ...
  if [[ "$__want_prompts" == "нет-слоя" ]]; then
    ...
    if (( __prompts > 0 || __nf > 0 )); then
      ...
      echo "  Замените «нет-слоя» на пол в третьем поле строки $__ver в" >&2
      echo "    $TWEAKCC_EXPECTED_APPLIED" >&2
      return 1
    fi
  elif [[ ! "$__want_prompts" =~ ^[0-9]+$ ]]; then
    echo "FATAL: в $TWEAKCC_EXPECTED_APPLIED пол промтов для $__ver не число и не «нет-слоя»: «${__want_prompts}»." >&2
    return 1
  ...
  echo "NOTE: уровень tweakcc на $__ver сошёлся: код $__code (объявлено $__want_code), промты $__prompts (пол $__want_prompts)" >&2
  return 0
}
...
print(v if isinstance(v, str) else "")' "$TWEAKCC_CFG" 2>/dev/null || true)"
  ...
  if "${TWEAKCC[@]}" --list-patches >"$TWEAKCC_LIST_OUT" 2>&1; then
    ...
    # тоже проходит здесь -- гашение объявляется внутри функции.
