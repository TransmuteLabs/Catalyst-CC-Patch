...
__tw_check_applied_level() {
  ...
  local __outage __outage_dl __out_exists
  ...
  # человек. Снимок промтов апстрима качается ПО ВЕРСИИ, и на свежем релизе его
  # может ещё не быть: форк печатает свою пару строк и идёт дальше, а все три
  # счёта слоя выходят нулями -- ровно теми же, что у машины, где слоя нет
  # вовсе. Ветка ниже читала бы (0,0,0) как «сходится» и пропускала бы сборку,
  # у которой не легло НИ ОДНОЙ накладки; а её соседка предлагала бы вписать
  ...
  #
  # Код 2, а не 1: по таблице кита это «прибор не может мерить, повтор
  # поможет». Здесь он буквален -- слой не измерен, потому что мерить было
  # нечего, и тот же прогон на той же версии зазеленеет сам, как только
  # апстрим опубликует снимок. Красить это отказом сборки значило бы сказать,
  # что сломана версия.
  __out_exists=1
  ...
  if (( __outage > 0 || __outage_dl > 0 )); then
    ...
    echo "  Снимок качается по версии; проверить и повторить, когда появится:" >&2
    echo "    https://raw.githubusercontent.com/Piebald-AI/tweakcc/refs/heads/main/data/prompts/prompts-$__ver.json" >&2
    return 2
  fi
