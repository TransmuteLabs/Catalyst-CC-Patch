...
__tw_reconcile_misses() {
  ...
  # крестиков слой промтов не печатает, а предмет этой сверки -- правки кода.
...
# ЕДИНСТВЕННЫЙ дом разбора вывода tweakcc: один вывод читают пять дверей, и
# разошедшийся разбор дал бы им разные множества -- объявление дома сходилось бы
# с одним из них через раз. LC_ALL=C на каждой ступени: отметки не-ASCII, а
# разбор под локалью машины считает мимо (измерено на этом же ките).
# <вывод> <code|prompt> <класс отметок, напр. '✓' или '✓✗○'> -> ИМЕНА по строке
__tw_layer_names() {
  ...
  LC_ALL=C awk -v secs="$__secs" -v marks="$__marks" '
    BEGIN { n = split(secs, s, "|"); for (i = 1; i <= n; i++) want[s[i]] = 1 }
    # Заголовок секции -- ровно два пробела и двоеточие в конце; строка правки --
    # ровно четыре пробела и отметка. Заголовки повторяются при КАЖДОМ вызове
    # tweakcc: секция переустанавливается, состояние между вызовами не копится.
    ...
    /^    (✓|✗|○) / {
      ...
      # sub, а не substr: длина отметки в байтах и в символах разная, и разбор
...
# Часовой формы вывода: строки правок, встреченные ДО первого заголовка либо под
# заголовком, которого нет ни в одном перечислении секций. Такая строка не
# принадлежит НИ ОДНОМУ слою, и молчаливое отнесение её к любому вернуло бы тот
# самый отказ по чужой причине, ради снятия которого слои и различаются секцией.
# <вывод> -> строки ✓/✗/○ вне известных секций
__tw_unsectioned_lines() {
  LC_ALL=C awk -v secs="${__TW_CODE_SECTIONS}|${__TW_PROMPT_SECTIONS}" '
    BEGIN { n = split(secs, s, "|"); for (i = 1; i <= n; i++) want[s[i]] = 1 }
    /^  [^ ].*:$/ { cur = $0; sub(/^  /, "", cur); sub(/:$/, "", cur); next }
...
__tw_check_applied_level() {
  ...
  local __unsect __floor_rows __floor_said
  ...
  # Расклад попытки: легло + выключено конфигурацией + не легло. Одно число
  # попыток не отличает просевший форк от чужого конфига, поэтому дверь
  # печатает расклад, а гейтит сумму.
  __off="$(__tw_off_code_names "$__out" | LC_ALL=C grep -a -c . || true)"
  __off="${__off//[^0-9]/}"
  [[ -n "$__off" ]] || __off=0
  __failed=$(( __tried - __code - __off ))
  if (( __nf > 0 )); then
  ...
  if [[ "${CLAUDE_PATCH_ALLOW_TWEAKCC_FAILURES:-0}" == "1" ]]; then
    ...
    # молча, неотличима от двери, которая держится.
  ...
  # одному счёту. Отказ стоит ПЕРВЫМ среди отказов: на разъехавшемся выводе
  # любое число ниже посчитано не о том.
  if [[ -n "$__unsect" ]]; then
    echo "FATAL: строки правок tweakcc вне известных секций вывода:" >&2
    printf '%s\n' "$__unsect" | LC_ALL=C sed -n '1,10s/^/  /p' >&2 || true
    echo "  форк tweakcc сменил форму печати или завёл незнакомую группу; слои" >&2
    echo "  различаются по секции, и разойтись по счётам молча они не должны." >&2
    return 1
  fi
  # Версия -- из БАЙТОВ образа, по той же причине, что и в сверке непроходов:
  # переменная выше по тексту присваивается только на одной ветке.
  __ver="$(LC_ALL=C grep -a -o -m1 '// Version: [0-9][0-9.]*' "$__bin" 2>/dev/null \
           | head -1 | sed 's|// Version: ||')"
  if [[ ! "$__ver" =~ ^[0-9]+\.[0-9]+\.[0-9]+ ]]; then
  ...
  __rows="$(awk -F'\t' -v v="$__ver" '
      /^[[:space:]]*#/ { next } NF==0 { next }
      { k=$1; gsub(/^[[:space:]]+|[[:space:]]+$/,"",k); if (k==v) n++ }
      END { print n+0 }
    ' "$TWEAKCC_EXPECTED_APPLIED" 2>/dev/null)"
  if (( __rows > 1 )); then
  ...
  if (( __tried < __want_tried )); then
    ...
    echo "  Крестиков при этом может не быть ни одного: когда данных под версию" >&2
    echo "  нет, правки не пробуются вовсе -- попытка исчезает из вывода целиком," >&2
    echo "  и сверка непроходов такую версию считает чистой. Разберитесь с" >&2
  ...
  if (( __tried > __want_tried )); then
    ...
    echo "  Объявление пережило причину -- реестр форка вырос, а запись" >&2
    echo "  продолжала бы прикрывать провал. Поднимите число в" >&2
    echo "    $TWEAKCC_EXPECTED_APPLIED" >&2
    return 1
  fi
  ...
  if [[ ! -f "$TWEAKCC_EXPECTED_PROMPT_FLOOR" ]]; then
    ...
    if (( __prompts == 0 && __nf == 0 )); then
      echo "NOTE: пол слоя промтов tweakcc для дома $TWEAKCC_HOME не объявлен, и слоя нет (легло 0, не найдено 0) -- сходится" >&2
    ...
    if [[ "$__want_prompts" == "нет-слоя" ]]; then
      ...
      if (( __prompts > 0 || __nf > 0 )); then
        echo "FATAL: на $__ver слой промтов объявлен отсутствующим, но он ожил: легло $__prompts, не найдено $__nf." >&2
  ...
  echo "NOTE: уровень tweakcc на $__ver сошёлся: код $__tried попыток (объявлено $__want_tried в $TWEAKCC_EXPECTED_APPLIED; легло $__code, выключено конфигурацией $__off, не легло $__failed), промты $__prompts ($__floor_said)" >&2
  return 0
}
...
__tw_check_off_set() {        # <вывод tweakcc> <образ> -> 0|1
  local __out="$1" __bin="$2"
  local __ver __off __measured __declared __undeclared __stale __bad=0
  __off="$(__tw_off_code_names "$__out" | LC_ALL=C grep -a -c . || true)"
  __off="${__off//[^0-9]/}"
  [[ -n "$__off" ]] || __off=0
  if [[ "${CLAUDE_PATCH_ALLOW_TWEAKCC_FAILURES:-0}" == "1" ]]; then
  ...
  if [[ ! -f "$TWEAKCC_EXPECTED_OFF" ]]; then
    ...
    # обратно. Отказ здесь красил бы машину, на которой ничего не сломано --
    # свежую и клон-пустышку дома, который свип создаёт сам, когда живого нет.
    if (( __off == 0 )); then
  ...
  if [[ -n "$__undeclared" ]]; then
    ...
    echo "  Конфиг дома изменился без объявления. Либо объявите их в" >&2
    echo "    $TWEAKCC_EXPECTED_OFF" >&2
  ...
  if [[ -n "$__stale" ]]; then
    ...
    printf '%s\n' "$__stale" | sed 's/^/  /' >&2
    echo "  Объявление пережило причину -- конфиг включил их обратно. Снимите строки из" >&2
    echo "    $TWEAKCC_EXPECTED_OFF" >&2
...
print(v if isinstance(v, str) else "")' "$TWEAKCC_CFG" 2>/dev/null || true)"
  ...
  if "${TWEAKCC[@]}" --list-patches >"$TWEAKCC_LIST_OUT" 2>&1; then
    ...
    # Дверь УРОВНЯ стоит вне цепочки выше: та ветвится по наличию ✗, а
    # проседание слоя случается ИМЕННО без крестиков. Ветка со слепой ручкой
    # тоже проходит здесь -- гашение объявляется внутри функции. Порядок
    # ОБЯЗАТЕЛЕН: сперва уровень (пропавшие попытки), потом множество
    # выключенных -- иначе при пропаже данных под версию вторая дверь кричала
    # бы всеми именами разом ПОВЕРХ настоящей причины.
    if ! __tw_check_applied_level "$TWEAKCC_OUT" "$BIN"; then
      rm -f "$TWEAKCC_OUT"
      exit 1
    fi
