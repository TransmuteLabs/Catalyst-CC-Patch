...
# <версия> <знак> <человеческое имя> <измеренные> <объявленные> -> 0 сошлось, 1 отказ
__tw_inert_check_sign() {
  local __v="$1" __s="$2" __human="$3" __got="$4" __want="$5" __miss __extra
  ...
  if [[ -n "$__extra" ]]; then
    echo "FATAL: на $__v объявлены правки tweakcc «${__human}», которых больше нет:" >&2
    printf '%s\n' "$__extra" | LC_ALL=C sed -n "1,10s/^/  $__s /p" >&2 || true
    echo "  Запись пережила свою причину: правка снова работает (или ушла из" >&2
    echo "  реестра форка), а строка продолжала бы прикрывать будущую пропажу." >&2
...
__tw_check_applied_level() {
  ...
  local __unsect __floor_rows __floor_said __floor_why
  local __pfail __noop __noop_n __vskip __vskip_n
  local __anchor_n __inert_dups __inert_nowhy __inert_bad
  # Якорь блока результатов снимается ПЕРВЫМ: он задаёт ОБЛАСТЬ всем счётам
  # ниже. При нуле якорей область пуста, каждый счёт даёт ноль, и дверь уровня
  # обвинила бы реестр форка вместо формы вывода; при двух в один счёт попали
  # бы две сборки. Сам ОТКАЗ -- ниже, в зоне отказов, вместе с соседями.
  __anchor_n="$(tw_results_anchor_count "$__out")"; __anchor_n="${__anchor_n//[^0-9]/}"
  [[ -n "$__anchor_n" ]] || __anchor_n=0
  ...
  if [[ "${CLAUDE_PATCH_ALLOW_TWEAKCC_FAILURES:-0}" == "1" ]]; then
    ...
    if (( __anchor_n != 1 )); then
      echo "NOTE: CLAUDE_PATCH_ALLOW_TWEAKCC_FAILURES=1 -- дверь якоря блока результатов tweakcc погашена (якорей $__anchor_n, а нужен ровно один)" >&2
    fi
    if [[ -n "$__unsect" ]]; then
      echo "NOTE: CLAUDE_PATCH_ALLOW_TWEAKCC_FAILURES=1 -- часовой формы вывода tweakcc погашен (строк вне известных секций либо с незнакомым знаком: $(printf '%s\n' "$__unsect" | LC_ALL=C grep -a -c . || true))" >&2
    fi
    if (( __pfail > 0 )); then
      echo "NOTE: CLAUDE_PATCH_ALLOW_TWEAKCC_FAILURES=1 -- дверь непрошедших накладок tweakcc погашена (не легло накладок $__pfail)" >&2
    fi
    # БЕЗ УСЛОВИЯ, как у двери уровня и двери выключенных: голос двери читает
    # не только человек, но и поле вердикта свипа, а поле, замолкающее на части
    # законных прогонов, отличить от снятой двери нельзя. Числа в строке и так
    # называют, что именно погашено -- нули говорят «гасить было нечего».
  ...
  # Якорь блока результатов -- ОТКАЗ ПЕРВЫМ среди отказов: он задаёт область
  # всем счётам, и без него каждое число ниже посчитано не о том. Двусторонне,
  # как и соседние двери: ноль якорей -- форк сменил строку, открывающую блок
  # (тогда область пуста и все счёта врут нулём); два и больше -- в один счёт
  # попали две сборки.
  if (( __anchor_n != 1 )); then
    echo "FATAL: якорь блока результатов tweakcc встречается $__anchor_n раз, а нужен ровно один." >&2
    echo "  Якорь: «${TW_RESULTS_ANCHOR}»" >&2
    if (( __anchor_n == 0 )); then
  ...
  # одному счёту. Отказ идёт сразу за якорем: на разъехавшемся выводе
  ...
  # Инертные правки (⊘ и ≡) гейтятся ниже, ПОИМЁННО и по версии: их дверь
  # стоит после извлечения версии, потому что набор принадлежит паре
  # «версия + реестр форка». Безусловного отказа на знак здесь нет намеренно --
  # см. комментарий у __tw_noop_code_names.
  ...
  if [[ -z "$(__tw_inert_rows "$__ver")" ]]; then
    ...
    __tw_inert_check_sign "$__ver" '⊘' 'пропущено по версии' \
      "$__vskip" "$(__tw_inert_declared "$__ver" '⊘')" || __inert_bad=1
    ...
    __tw_inert_check_sign "$__ver" '≡' 'отработало вхолостую' \
      "$__noop" "$(__tw_inert_declared "$__ver" '≡')" || __inert_bad=1
