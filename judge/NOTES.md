...
## 2026-08-30: замер флота — 152 консультации из 218 назвали бриф и не получили его

Волна 26 (вложение брифа, на который указывает диспатч) стоит в образе и
РАБОТАЕТ. Замер по живым записям судьи за 2026-08-30 (218 консультаций, 10
процессов), с разделением двух причин пустого вложения:

| что было в диспатче | вложение |  записей |
|---|---|---:|
| назвал существующий файл | ЕСТЬ | 14 |
| назвал существующий файл | нет | **152** |
| назвал путь, файла нет | нет | 26 |
| путей не назвал | нет | 26 |

Все 14 вложений (docnum:other -- это замер флота, а не счётчик кита) принёс
ОДИН процесс (84958) — и у него ни одного промаха:
14 из 14 случаев, где файл был назван, кончились вложением. Остальные восемь
процессов не приложили ни одного файла из 152 названных.

Причина не в механизме, а в возрасте образа: живые сессии исполняют
`versions/2.1.250` (проверено `lsof -p <pid>` по полю txt), тогда как
`~/.local/bin/claude` уже указывает на 2.1.251. Пока сессии не перезапущены,
судья в них судит УКАЗАТЕЛЬ на работу, а корпус пополняется записями, по
которым готовность брифа измерить нельзя.

Следствие для замеров: годный набор для оси «готовность брифа» набирается
только из записей ПОСЛЕ перезапуска. Реплика старых записей с чтением файлов
с диска эту дыру не закрывает — она показывает судье БУДУЩИЙ текст
(см. `Catalyst-Judge-Eval/docs/criterion.md`, раздел от 2026-08-30).

Перезапуск сессий — действие человека: это его терминалы и его контекст.

## 2026-08-30: потолок записей 5000 держит

На диске 764 записи, самая старая 2026-08-28T04:30 — окно перевалило за
прежние 500 и не подрезано. Это же служит положительным контролём: настройка
`records_keep` действительно читается исполняющимся образом, иначе умолчание
500 срезало бы корпус. Вес 34 МБ на 764 записи (44 КБ на запись в нынешней
смеси), то есть порядка 220 МБ на полном окне.
