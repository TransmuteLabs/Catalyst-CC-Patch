...
step('22 judge consulted before a subagent dispatch', () => {
  ...
  const core =
    ...
    'try{' +
      ...
      //
      // Потолок объявляется трижды: сколько файлов взято (attach_files),
      // сколько знаков от каждого (attach_chars) и сколько знаков суммарно
      // (attach_total). Третий нужен потому, что первые два не ограничивают
      // ПРОИЗВЕДЕНИЕ: поднять число файлов, не тронув остальное, значит молча
      // умножить нагрузку одной консультации. Суммарный ставится равным
      // произведению двух других — тогда поднятие любого упирается сразу, а
      // не через полгода.
      //
      // Числа — замер по 500 живым записям (Catalyst-Judge-Eval,
      // tools/census_paths.py), по ТЕКСТУ ДИСПАТЧА: путь называют 384 записи
      // из 500, существующий файл — 164, и больше ТРЁХ файлов не называет ни
      // одна (1→111, 2→51, 3→2). Размер файла: медиана 9462, p90 29079.
      // Наибольшая суммарная нагрузка на корпусе — 53356 знаков.
      // Умолчание ядра — НОЛЬ:
      ...
      'let __att=[];' +
      'if(__atn>0&&__atc>0&&__atb>0){let __sn={},__mm,__sp=0,' +
        ...
        'while((__mm=__rx.exec(__dsrc))&&__att.length<__atn){' +
          ...
          // Суммарный бюджет режет ПОСЛЕДНИЙ вошедший файл, а не выкидывает
          // его целиком: половина брифа отвечает на больше вопросов, чем его
          // отсутствие, а объявление усечения одно на обе причины — читателю
          // важно «показано N из M», а не чей потолок сработал.
          'let __rm=__atb-__sp;if(__rm<=0)break;' +
          'let __tk=__atc<__rm?__atc:__rm;' +
          'let __bc=__bd.length>__tk;' +
          'let __kn=__bc?__tk:__bd.length;__sp+=__kn;' +
          '__att.push({f:__f,n:__bd.length,c:__bc,k:__kn,' +
            't:__bc?__sur(__bd.slice(0,__tk)):__bd})}}' +
