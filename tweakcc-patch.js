...
step('22 judge consulted before a subagent dispatch', () => {
  ...
  const judge =
    ...
    'try{' +
      ...
      // Подрезка ленты. Три требования, каждое куплено инцидентом:
      // (1) носители распоряжений человека — его реплики и резюме компакции —
      //     не выбрасываются целиком, а режутся по тексту: выброшенный носитель
      //     это молча потерянное распоряжение (стенд, 2026-08-21);
      // (2) доля считается на КЛАСС, а не на запись: поштучный потолок резюме
      //     давал 64% ленты при задуманных 30%, и незакреплённое вытеснялось
      //     подчистую — судья переставал видеть сам ход работы;
      // (3) счёт идёт по числам, а не повторной сериализацией всего массива:
      //     прежняя редакция была квадратичной (4800 записей — 8.3 секунды
      //     ЧИСТОГО локального счёта при пороге ступени 25 секунд) и при
      //     исчерпании счётчика итераций отдавала ленту ВЫШЕ бюджета.
      'let __cut=(__n)=>{' +
        'let __b=Math.max(200,__n-180),__pb=Math.floor(__b*0.35),__sb=Math.floor(__b*0.3);' +
        'let __d=0,__dp=0,__c2=0;' +
        'let __cs=(__x)=>JSON.stringify(__x).length+1;' +
        'let __a=__arr.slice(),__w=__a.map(__cs),__tot=2;' +
        ...
        // разделы «все сообщения пользователя» и «незакрытые задачи».
        'let __tr=(__i,__lim)=>{let __t=String(__a[__i].text);if(__t.length<=__lim)return 0;' +
          'let __h=Math.floor(__lim*0.55),__tl=Math.max(0,__lim-__h-44);' +
          'let __nx={src:__a[__i].src,text:__t.slice(0,__h)+" [\\u0432\\u044b\\u0440\\u0435\\u0437\\u0430\\u043d\\u043e "+(__t.length-__h-__tl)+" \\u0437\\u043d\\u0430\\u043a\\u043e\\u0432] "+(__tl?__t.slice(-__tl):"")};' +
          'let __c=__cs(__nx),__g=__w[__i]-__c;if(__g<=0)return 0;' +
          '__a[__i]=__nx;__w[__i]=__c;__tot-=__g;__c2++;return __g};' +
        'let __sum=(__f)=>{let __r=0;for(let __k=0;__k<__a.length;__k++)if(__f(__a[__k]))__r+=__w[__k];return __r};' +
        'let __long=(__f)=>{let __i=-1,__L=-1;for(let __k=0;__k<__a.length;__k++)' +
          'if(__f(__a[__k])){let __x=String(__a[__k].text).length;if(__x>__L){__L=__x;__i=__k}}return __i};' +
        'let __cnt=(__f)=>{let __r=0;for(let __k=0;__k<__a.length;__k++)if(__f(__a[__k]))__r++;return __r};' +
        'let __del=(__i,__p)=>{__tot-=__w[__i];__a.splice(__i,1);__w.splice(__i,1);__d++;if(__p)__dp++};' +
        // Внутри доли класса сначала укорачивается САМАЯ ДЛИННАЯ запись, и только
        // когда резать нечего — выбрасывается самая давняя: размер не признак
        // важности, а возрастная политика выбрасывала пятнадцатизначный стоячий
        // запрет ради стотысячной пасты.
        'let __cap=(__f,__lim)=>{let __s1=__sum(__f);' +
          'for(let __g=0;__g<20000&&__s1>__lim;__g++){' +
            'let __i=__long(__f);if(__i<0)break;' +
            'let __L=String(__a[__i].text).length;' +
            'if(__L>400){let __gg=__tr(__i,Math.max(200,__L-(__s1-__lim)-88));' +
              'if(!__gg)break;__s1-=__gg;continue}' +
            'if(__cnt(__f)<=1)break;' +
            'let __j=-1;for(let __k=0;__k<__a.length;__k++)if(__f(__a[__k])){__j=__k;break}' +
            'if(__j<0)break;let __c=__w[__j];__del(__j,!0);__s1-=__c}};' +
        '__cap(__iss,__sb);__cap(__isu,__pb);' +
        // Незакреплённую запись, удаление которой увело бы ленту НИЖЕ бюджета,
        // нельзя выбрасывать целиком: зазор остаётся неиспользованным, а рабочий
        // контекст — выброшенным. В пределе лента обнулялась совсем (одна свежая
        // запись длиннее бюджета вытесняла всё, включая себя), и судья выносил
        // обычный вердикт вслепую — это хуже отмены, потому что незаметно.
        // Измерено стендом 2026-08-21: на ступени с лентой 12000 судья видел
        // 446 знаков, то есть 96% бюджета пропадало вместе с ходом работы.
        'for(let __k=0;__k<__a.length&&__tot>__b;){' +
          'if(__pr(__a[__k])){__k++;continue}' +
          'if(__tot-__w[__k]>=__b){__del(__k,!1);continue}' +
          'let __L=String(__a[__k].text).length;' +
          'if(__L>120){__tr(__k,Math.max(80,__L-(__tot-__b)-88));__k++;continue}' +
          '__del(__k,!1)}' +
        ...
        'for(let __g=0;__g<20000&&__tot>__b&&__a.length>1;__g++){' +
          'let __i=__long(()=>!0);if(__i<0)break;' +
          'let __L=String(__a[__i].text).length;' +
          'if(__L>240){if(!__tr(__i,Math.max(200,__L-(__tot-__b)-88)))break;continue}' +
          '__del(0,__pr(__a[0]))}' +
        'if(__tot>__b&&__a.length===1){__a=[{src:__a[0].src,' +
          'text:String(__a[0].text).slice(-Math.max(0,__b-100))}];__w=[__cs(__a[0])];__tot=2+__w[0]}' +
        ...
        'if(__d>0||__c2>0)__a.unshift({src:"injected",text:"[\\u043b\\u0435\\u043d\\u0442\\u0430 \\u043f\\u043e\\u0434\\u0440\\u0435\\u0437\\u0430\\u043d\\u0430: \\u0432\\u044b\\u0442\\u0435\\u0441\\u043d\\u0435\\u043d\\u043e "+__d+" \\u0437\\u0430\\u043f\\u0438\\u0441\\u0435\\u0439; \\u0437\\u0430\\u043a\\u0440\\u0435\\u043f\\u043b\\u0435\\u043d\\u043e \\u0440\\u0435\\u043f\\u043b\\u0438\\u043a \\u0447\\u0435\\u043b\\u043e\\u0432\\u0435\\u043a\\u0430: "+__cnt(__isu)+", \\u0440\\u0435\\u0437\\u044e\\u043c\\u0435 \\u043a\\u043e\\u043c\\u043f\\u0430\\u043a\\u0446\\u0438\\u0438: "+__cnt(__iss)' +
          '+(__dp?"; \\u0412\\u042b\\u0422\\u0415\\u0421\\u041d\\u0415\\u041d\\u041e \\u0417\\u0410\\u041a\\u0420\\u0415\\u041f\\u041b\\u0401\\u041d\\u041d\\u042b\\u0425: "+__dp:"")+(__c2?"; \\u043f\\u043e\\u0434\\u0440\\u0435\\u0437\\u0430\\u043d\\u043e \\u043f\\u043e \\u0442\\u0435\\u043a\\u0441\\u0442\\u0443: "+__c2:"")+"]"});' +
