  ...
  const judge =
    ...
    'try{' +
      ...
      'let __cut=(__n)=>{let __a=__arr.slice(),__s=JSON.stringify(__a),__d=0;' +
        ...
        // своя доля ленты. Носители распоряжений человека — его реплики И резюме
        // компакции — режутся ПО ТЕКСТУ и никогда не выбрасываются целиком:
        // выбросить носитель значит потерять распоряжение молча. Асимметрия
        // «резюме режем, реплику выбрасываем» уже давала дыру (стенд, 2026-08-21:
        // одна большая вставка юзера выдавливала и резюме, и сам запрет, а
        // маркер при этом рапортовал «закреплено реплик человека: 1»).
        ...
        'let __tr=(__x,__lim)=>{let __t=String(__x.text);if(__t.length<=__lim)return __x;' +
          'let __h=Math.floor(__lim*0.55),__tl=Math.max(0,__lim-__h-40);__c2++;' +
          'return{src:__x.src,text:__t.slice(0,__h)+" [\\u0432\\u044b\\u0440\\u0435\\u0437\\u0430\\u043d\\u043e "+(__t.length-__h-__tl)+" \\u0437\\u043d\\u0430\\u043a\\u043e\\u0432] "+(__tl?__t.slice(-__tl):"")}};' +
        'let __pb=Math.floor(__b*0.35),__sb=Math.floor(__b*0.3),__c2=0,__dp=0;' +
        'let __pr=(__x)=>__x&&(__x.src==="user"||__x.src==="compaction-summary");' +
        'let __us=()=>__a.filter((__x)=>__x&&__x.src==="user");' +
        '__a=__a.map((__x)=>__x&&__x.src==="compaction-summary"?__tr(__x,__sb):__x);' +
        // Реплики человека внутри своей доли режутся ПО ДЛИНЕ, а не по возрасту:
        // возрастная политика выбрасывала пятнадцатизначный стоячий запрет, чтобы
        // сохранить стотысячную пасту (стенд, 2026-08-21). Размер не признак
        // важности, поэтому сначала укорачивается САМАЯ ДЛИННАЯ реплика — так
        // все распоряжения остаются в ленте хотя бы урезанными. Выбрасывание по
        // старшинству включается, только когда резать уже нечего.
        'for(let __g=0;__g<60&&JSON.stringify(__us()).length>__pb;__g++){' +
          'let __i=-1,__ln=0;' +
          'for(let __k=0;__k<__a.length;__k++){if(__a[__k]&&__a[__k].src==="user"){' +
            'let __L=String(__a[__k].text).length;if(__L>__ln){__ln=__L;__i=__k}}}' +
          'if(__i<0)break;' +
          'if(__ln>400){let __ov=JSON.stringify(__us()).length-__pb;' +
            '__a[__i]=__tr(__a[__i],Math.max(200,__ln-__ov-80));continue}' +
          'if(__us().length<=1)break;' +
          'let __j=__a.findIndex((__x)=>__x&&__x.src==="user");' +
          'if(__j<0)break;__a.splice(__j,1);__d++;__dp++}' +
        ...
        // Последний рубеж: незакреплённого не осталось, а бюджет всё ещё
        // превышен. Раньше здесь стоял безусловный shift, который молча сносил
        // ровно то, что закреплено выше. Теперь режется САМЫЙ ДЛИННЫЙ носитель,
        // и только если резать больше нечего — выбрасывается голова.
        'for(let __g=0;__g<40&&__a.length>1&&__s.length>__b;__g++){' +
          'let __i=-1,__ln=0;' +
          'for(let __k=0;__k<__a.length;__k++){let __L=String(__a[__k]?.text||"").length;' +
            'if(__L>__ln){__ln=__L;__i=__k}}' +
          'if(__i<0||__ln<=240){__a.shift();__d++;__dp++;__s=JSON.stringify(__a);continue}' +
          '__a[__i]=__tr(__a[__i],Math.max(200,__ln-(__s.length-__b)-80));__s=JSON.stringify(__a)}' +
        'if(__s.length>__b&&__a.length===1){__a=[{src:__a[0].src,' +
          'text:String(__a[0].text).slice(-Math.max(0,__b-100))}];__s=JSON.stringify(__a)}' +
        // Маркер обязан говорить и о ПОТЕРЯННОМ среди закреплённого: счёт одних
        // выживших выглядит благополучно ровно в тот момент, когда распоряжение
        // вытеснено (тот же успокоительный счёт, что и прежняя формулировка).
        'if(__d>0||__c2>0){let __ku=__us().length,' +
