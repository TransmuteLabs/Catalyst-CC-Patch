...
# ЗУБ 2: префикс сохраняет $bunfs.
# Красный: собрать префикс без этой подстроки.
# Если bun --version при этом не меняется наблюдаемо — находка, зуб не подгонять.
tooth_2() {
  ...
  # префикс без $bunfs: зуб красный. Поведение bun — отдельно, как находка.
  run_tool "$out" "$err" run --tree "$tree" -- --version
  rc=$?
  if grep -q -F '2.1.273 (Claude Code)' "$out" && [[ $rc -eq 0 ]]; then
    say "НАХОДКА: префикс без \$bunfs, bun --version rc=0 — предикат xg() не единственный путь резолва для --version (обычный FS-лоадер bun принимает абсолютный путь)"
  else
    say "префикс без \$bunfs: bun --version rc=$rc (предикат xg наблюдаем)"
  fi
