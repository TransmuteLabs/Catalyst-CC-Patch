...
sha256_of() {
  local out
  # Пустой хеш — штатный отказ чтения файла, его сообщает return 1 ниже;
  # 2>/dev/null на строке не снимать (уже стоял).
...
# `6>&-`: дети наследуют дескриптор замка -- см. гейт tools/lockfd-check.py.
# Платформа ЦЕЛИ передаётся ЯВНО, по той же причине, что и в свипе: наполнитель
# качает из реестра для ЭТОЙ машины, значит цель здесь -- хозяин, и это видно в
# команде, а не спрятано в умолчании разборщика.
FETCH_PLATFORM=$(cd "$KIT" && python3 -c 'import claude_patch; print(claude_patch.npm_platform_pkg())' 6>&-) || {
  echo "ОТКАЗ: не определить платформу этой машины (claude_patch не грузится) -- сломан кит, а не список" >&2
  exit 2; }
ENTRIES=$(python3 "$KIT/tools/corpus-list.py" "$LIST" "$FETCH_PLATFORM" 6>&-) || {
  __lrc=$?
  if (( __lrc == 2 )); then
