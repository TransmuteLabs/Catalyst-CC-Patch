...
# Зубы реестра проверок -- третья половина предполёта.
#
# Пол проверок доказывает, что проверка КРАСНА на стоке; это не то же самое,
# что «краснеет, когда свойство умирает». Аудит нашёл проверки, чьё имя обещало
# а тело пинило токен, -- и ни одна из них не могла упасть.
# Прибор мутирует СОБРАННЫЙ образ и требует, чтобы каждая мутация покраснила
# ровно объявленные двери. Образа на машине нет -- объявленный пропуск (5), а не
# отказ: на чистой машине мерить нечего.
...
if (( __env_rc == 0 )); then
  ...
  python3 "$TEETH_PY" 8>&- > "$TEETH_LOG" 2>&1
  __trc=$?
  case $__trc in
    ...
    3) # Не красный прогон, а НЕИЗМЕРЕННЫЙ: живая сборка меняет образ под
       ...
       exit 3 ;;
    2) echo "SWEEP ОТКАЗ: прибор зубов НЕ МЕРИЛ: якорь мутации, длина замены или контроль (лог $TEETH_LOG)" >&2
       tail -6 "$TEETH_LOG" >&2
