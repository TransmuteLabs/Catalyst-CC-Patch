...
# сводке), SWEEP_LAST_N (сколько САМЫХ НОВЫХ версий
...
if [[ -n "${SWEEP_SKIP_BUILD_PROBE:-}" ]]; then
  ...
  case $__prc in
    ...
    2) echo "SWEEP ОТКАЗ: зонд пути сборки НЕ МЕРИЛ: сломан контракт его вызова или прибор внутри него не может мерить (лог $PROBE_LOG)" >&2
       tail -5 "$PROBE_LOG" >&2
       echo "# зонд пути сборки: НЕ ИЗМЕРЕН -- прибор не может мерить" >> "$STATE/log/sweep-summary.txt"
       exit 2 ;;
    3) echo "SWEEP ОТКАЗ: замок конвейера занят другим прогоном -- зонд не мерил (лог $PROBE_LOG)" >&2
       ...
       echo "# зонд пути сборки: НЕ ИЗМЕРЕН -- замок конвейера занят" >> "$STATE/log/sweep-summary.txt"
       exit 3 ;;
    6) echo "SWEEP ОТКАЗ: машинерия замка сломана -- зонд не мерил (лог $PROBE_LOG)" >&2
       echo "  Повтор не поможет: чинить окружение." >&2
       echo "# зонд пути сборки: НЕ ИЗМЕРЕН -- машинерия замка сломана" >> "$STATE/log/sweep-summary.txt"
       exit 6 ;;
...
echo "# sync цен: ПРОПУЩЕН во всех сборках (CLAUDE_PATCH_SKIP_MODELS=1)" >> "$STATE/log/sweep-summary.txt"
...
echo "# набор: $SET_NOTE" >> "$STATE/log/sweep-summary.txt"
...
if [[ "$CORPUS" != "$HOME/.local/share/claude-patch/corpus" || "$LIST" != "$SWEEP_KIT/tools/corpus-versions.txt" ]]; then
  ...
  echo "# корпус: НЕ БОЕВОЙ -- образы $CORPUS, список $LIST" >> "$STATE/log/sweep-summary.txt"
fi
...
if [[ -n "${SWEEP_SKIP_TOOLS_BENCH:-}" ]]; then
  ...
  echo "# стенд корпусных инструментов: ПРОПУЩЕН по ручке" >> "$STATE/log/sweep-summary.txt"
  ...
  case $__brc in
    0) echo "SWEEP стенд корпусных инструментов: ЗЕЛЁНО (лог $BENCH_LOG)"
       echo "# стенд корпусных инструментов: ЗЕЛЁНО" >> "$STATE/log/sweep-summary.txt" ;;
    2) echo "SWEEP ОТКАЗ: стенд корпусных инструментов НЕ МЕРИЛ: неизвестный режим или сломан контракт вызова (лог $BENCH_LOG)" >&2
       tail -5 "$BENCH_LOG" >&2
       echo "# стенд корпусных инструментов: НЕ ИЗМЕРЕН -- прибор не может мерить" >> "$STATE/log/sweep-summary.txt"
       exit 2 ;;
    6) echo "SWEEP ОТКАЗ: машинерия замка стенда сломана -- стенд не мерил (лог $BENCH_LOG)" >&2
       tail -5 "$BENCH_LOG" >&2
       echo "# стенд корпусных инструментов: НЕ ИЗМЕРЕН -- машинерия замка сломана" >> "$STATE/log/sweep-summary.txt"
       exit 6 ;;
    3) echo "SWEEP ОТКАЗ: стенд корпусных инструментов занят -- его замок держит другой прогон стенда либо идёт настоящая сборка (лог $BENCH_LOG)" >&2
       ...
       tail -5 "$BENCH_LOG" >&2
       echo "# стенд корпусных инструментов: НЕ ИЗМЕРЕН (занято)" >> "$STATE/log/sweep-summary.txt"
       exit 3 ;;
    *) echo "SWEEP ОТКАЗ: стенд корпусных инструментов не сошёлся (код $__brc), лог $BENCH_LOG" >&2
       tail -5 "$BENCH_LOG" >&2
       echo "# стенд корпусных инструментов: КРАСНЫЙ (код $__brc)" >> "$STATE/log/sweep-summary.txt"
       exit 1 ;;
  esac
fi
...
if [[ -n "${SWEEP_SKIP_CHECKS_TEETH:-}" ]]; then
  ...
  echo "# зубы реестра проверок: ПРОПУЩЕНЫ по ручке" >> "$STATE/log/sweep-summary.txt"
  ...
  case $__trc in
    0) echo "SWEEP зубы реестра проверок: ЗЕЛЁНО (лог $TEETH_LOG)"
       echo "# зубы реестра проверок: ЗЕЛЁНО" >> "$STATE/log/sweep-summary.txt" ;;
    5) echo "SWEEP зубы реестра проверок ПРОПУЩЕНЫ: на машине нет собранного образа"
       echo "# зубы реестра проверок: ПРОПУЩЕНЫ -- нечего мерить" >> "$STATE/log/sweep-summary.txt" ;;
    2) echo "SWEEP ОТКАЗ: прибор зубов НЕ МЕРИЛ: якорь мутации, длина замены или контроль (лог $TEETH_LOG)" >&2
       tail -6 "$TEETH_LOG" >&2
       echo "# зубы реестра проверок: НЕ ИЗМЕРЕНЫ -- прибор не может мерить" >> "$STATE/log/sweep-summary.txt"
       exit 2 ;;
    4) echo "SWEEP ОТКАЗ: таблица мутаций реестра не той длины, чем объявлено (лог $TEETH_LOG)" >&2
       tail -4 "$TEETH_LOG" >&2
       echo "# зубы реестра проверок: НЕ ИЗМЕРЕНЫ -- расхождение объявленного числа" >> "$STATE/log/sweep-summary.txt"
       exit 1 ;;
    *) echo "SWEEP ОТКАЗ: проверка прошла мутацию молча либо покраснела чужой дверью (код $__trc, лог $TEETH_LOG)" >&2
       tail -8 "$TEETH_LOG" >&2
       echo "# зубы реестра проверок: КРАСНЫЕ (код $__trc)" >> "$STATE/log/sweep-summary.txt"
       exit 1 ;;
  esac
fi
...
for entry in "${SRC[@]}"; do
  ...
  printf '%s exit=%s ok=%s fail=%s tweakcc=%s ours=%s smoke=%s iface=%s bench=%s forms=%s floor=%s%s\n' \
