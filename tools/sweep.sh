...
#   3  замок свипа держит другой живой свип -- повторить позже
...
# сводке). Внутренние, руками не ставятся: SWEEP_LEADER, SWEEP_SELF, SWEEP_KIT.
set -u
...
if [[ -n "${SWEEP_SKIP_TOOLS_BENCH:-}" ]]; then
  ...
  case $__brc in
    ...
    6) echo "SWEEP ОТКАЗ: машинерия замка стенда сломана -- стенд не мерил (лог $BENCH_LOG)" >&2
       ...
       exit 6 ;;
    3) echo "SWEEP ОТКАЗ: стенд корпусных инструментов не смог мерить -- занят его замок или идёт настоящий прогон (лог $BENCH_LOG)" >&2
       tail -5 "$BENCH_LOG" >&2
...
for entry in "${SRC[@]}"; do
  ...
  if (( rc != 3 )); then
    ...
    if (( rc != 0 )); then
      case $rc in
        2) why+=("конвейер вернул 2: контракт вызова нарушен") ;;
