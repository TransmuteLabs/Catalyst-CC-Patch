...
EXPECTED_SCENARIOS=194
EXPECTED_MUTATIONS=252
...
scenario_148() {   # самопроверка не засчитывает зуб БЕЗ КОНТРОЛЯ
  ...
  # вложенный полный `--self-check` -- это все 252 мутаций corpus-tools-bench
...
scenario_192() {   # САЙТЫ ВЫЗОВА: диагност и классификатор действительно позваны
  ...
  # сравнения версий), версию из байтов -- три двери tweakcc.
  ...
  if (( bytes_n != 3 )); then
    ...
    bad "192 сайты вызова: дом версии из байтов позван $bytes_n раз, а дверей три"; return
  fi
