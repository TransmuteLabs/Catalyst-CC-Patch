...
def make_prefix(tree):
    ...
    # Предикат standalone-режима в chunk-m50792n2.js:
    #   xg(n){return n.includes("$bunfs")||n.includes("~BUN")||n.includes("/snapshot/")||n.startsWith("node:")}
    # без $bunfs (и без ~BUN /snapshot/ / node:) путь перестаёт считаться
    # виртуальной ФС. Суффикс поэтому не «/root/», а '/$bunfs/root/'.
    prefix = (tree_abs + BUNFS_ROOT_SUFFIX).encode('utf-8')
    if b'$bunfs' not in prefix:  # MUT_BUNFS_CHECK
        raise InstrumentError(
            'ПРИБОР: префикс не содержит $bunfs — предикат standalone ложен: {!r}'.format(
