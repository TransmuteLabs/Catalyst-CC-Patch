...
BLOCK="$(mktemp)" || { printf 'ПРИБОР НЕДОСТУПЕН: не создан временный файл блока проверок\n' >&2; exit 2; }
[ -n "$BLOCK" ] || { printf 'ПРИБОР НЕДОСТУПЕН: путь временного файла блока проверок пуст\n' >&2; exit 2; }
