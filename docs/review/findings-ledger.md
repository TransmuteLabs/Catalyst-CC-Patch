...
## 2026-09-04 — переход на 2.1.259: слой промтов вернулся, шаг 10 получил вторую форму

Реестр npm показал 2.1.259 при установленной 2.1.258; слово юзера «работай».
Пин корпуса из байтов реестра: `884baa38fe1a624be25c4a91568bf5a08b5cf4e7d7acf29b7760e3525d964898`.

Слой промтов tweakcc. `prompts-2.1.259.json` у апстрима ЕСТЬ (после 404 на
257/258) — тонкий слой двух прошлых версий подтверждён внешним окончательно:
на 259 счёт вернулся к 34 ✓ без единой правки кита. Но файл данных апстрима
частично протух против реального cli.js 259: 24 «Could not find system
prompt» (семья Skill: Artifact *, Data Visualization, Plan Artifact, Run *,
Claude Code configuration guide) + 3 «Skipped Agent Prompt» (stale prompt
file по словам самого tweakcc). Мягкие пропуски, не ✗; сторона tweakcc.
Для задачи #58 это третий вид сигнала, который печатаемое `tweakcc=N`
не различает: ✓ (легло), ✗ (не легло, гейтится домом непроходов), мягкий
пропуск (не считается вообще).

Прогоны. (1) — `IncompleteRead` на скачивании (сеть), ничего не установлено.
(2) — штатный FATAL по необъявленному `Clear screen command` (объявлен ряд
2.1.259, та же правка, что на 257/258). (3) — сверка зелёная, но не лёг наш
шаг 10 (per-model context window): 259 переписала тело помощника окна.
(4) — ЗЕЛЕНО.

Шаг 10 — вторая форма тела помощника. 2.1.251-258:
`function Gw(e){let t=Ot(e);return!t.startsWith("claude-")&&!PMe(Ye(t))}`;
2.1.259: `function OL(e){let n=Et(e),r=cr(n),o=ze(r);
if(bge(o)||r!==n&&bge(ze(n)))return!1;let p=r.toLowerCase();
return!p.startsWith("claude-")||p!==o}`. Ключ, который шаг читает из тела,
один по устройству в обеих формах: `canonical(parse(model))` — на 259 это
`cr(Et(e))`. Тело пинится ЦЕЛИКОМ в обеих формах (частичное совпадение =
имя от одной формы, смысл другой), выбор через `||`, группы 3=parse и
5=canonical совпадают — остальной код шага не тронут. Замеренная ловушка
подхода «по имени»: имена минификата чанк-локальны — в образе 259 ЧЕТЫРЕ
разные функции `OL` (и в 258 четыре `TL`); только тело целиком отличает
помощника окна от тёзок из других чанков. Контроли до правки: новая форма
матчит 259 (Et/cr) и не матчит 258; старая — наоборот (At/ze); локальный
прогон tweakcc-patch.js пиннутым форком по копии staging — все 25 шагов
легли, прелудия `__ccw?.[e] ?? __ccw?.[cr(Et(e))]` на месте.

Прогон 4: **код 0, 119 OK / 0 FAIL**; стенды судьи 44/0, цен 12/0 (+16/16),
проб 7/0 (+8/8); перепись 40/40; зубы гейта чисел 40/40; tweakcc 34 ✓ +
1 объявленный непроход с NOTE; пол проверок 3/119, все объявлены; интерфейс
поднялся; зонды 82/82, самопроверка 5/5. Установлено `mv` в
`versions/2.1.259`, ссылка перекинута; `claude --version` = 2.1.259.
Уборка собрала staging-сироту прогона 3; 2.1.258 оставлен (его исполняют
живые сессии, включая эту).

Вне конвейера, названо юзеру: `versions/2.1.257`, `2.1.257.orig` и
`2.1.258.orig` исчезли с диска между 03.09 21:31 и 04.09 00:15 (живых
сессий на 257 к тому моменту уже не было). Не уборка конвейера (она идёт
только на зелёном --update, зелёных в окне не было). Откат 258 к стоку
теперь только из корпуса (`2.1.258.pristine` цел, пин сходится) — покрытие
есть, но дубликат в versions/ был дешевле; записано как наблюдение, виновник
не установлен.

Свип каденции (набор становится `251 252 257 258 259`) — результат
дописывается ниже после прогона.

## Задача #52 — метки объёма возвратов судьи: ось «готовность брифа» (04.09)

Измерение, не гейт. Корпус записей судьи **волатилен**: утром 666 файлов,
к моменту второй оси — 365. Замер «666 записей» с диска больше не
воспроизводится; отсюда правило: **любой размеченный набор снимается
снимком до разметки**, иначе цифры первой оси нечем перепроверить.

**Ось веера (по 666 записям):** ok 571 / warn 63 / block 33, возвратов 97;
веер помянут в 47 из 97; из 13 BLOCK «по вееру» веер РЕАЛЬНО был у всех 13
(6 — соседи до, 7 — другой агент после), WARN 15 из 34. Это дефект #61
(бриф `docs/review/wave36-brief-fan-observable.md`), а не разметка объёма.

**Ось «готовность брифа» (по текущим 365 записям, 67 возвратов):** фильтр
по ключевым словам дал 15 BLOCK, но личное чтение вердиктов показало:
**настоящих возвратов по готовности брифа — 4**, остальные 11 — ось линзы
и модели. Все 4 обоснованы; три проверены ДОСЛОВНО по самому предмету:

- `…-000384` — диспатч: «REPORT: return it as TEXT in your final message
  — do not create a report file»; бриф §8: «Write `<scratchpad>/636b-report.md`,
  ending with the literal line `END OF REPORT`». Два авторитетных указания
  о доставке отчёта противоречат друг другу — исполнить оба нельзя.
- `…-7989-000006` — класс D обещает «(52 файла, список ниже)»; вложение
  (6761 симв., пометок подреза НЕТ) идёт от этого абзаца сразу к
  «## Точечные находки». Перечня нет физически.
- `…-64746` (гейты) — бриф требует держать гейты на 80 со стоп-правилом
  BLOCKED, тогда как диспатч и лента показывают 82.
- `…-7989-000007` — переиздание: обещание списка заменено критерием, но
  перечня по-прежнему нет; судья ссылается на прогон в ленте, где критерий
  границу не воспроизводит. Обоснован, но самый строгий из четырёх.

**Вердикт оси:** «готовность брифа» ЗДОРОВА — возвраты точные и заземлённые
в предмете, правка механизма не нужна. Дефектна только ось веера (#61).

**Ловушки прибора, измеренные здесь** (обе давали ложный результат до
исправления): (1) в записи БОЛЬШЕ ОДНОГО заголовка `=== ФАЙЛ` — три штуки,
два из них внутри пересказа ленты; брать надо ПОСЛЕДНИЙ, иначе режется
не вложение; (2) проверку пометки подреза гнать по ИЗОЛИРОВАННОМУ вложению
— поиск подстроки по всей записи ловит слово из системного промта.
Текст промта лежит в `request.messages`, ключа `messages` верхнего уровня
у записи нет.

## Волна 35 (04.09) — три двери формы: #56 числа в heredoc'ах, #58 уровень tweakcc, #59 замок по наследству

Одно дерево, три предмета; общий у них только файл конвейера, поэтому и
коммит один. Свип каденции гоняется на этом состоянии отдельно (раздел ниже).

### #56 — гейт чисел читает питоновские heredoc'и

Раньше блок `python3 <<'PY'` был для гейта одной сплошной строкой данных:
комментарии внутри проходили как комментарии оболочки, докстринги не
читались вовсе, и число, напечатанное прибором ИЗ такого блока, гейт не
видел by construction. Теперь блоки размечаются до основного цикла
(`heredoc_map`): открытие ловится по `<<-?\s*'?ТЕГ'?$` И упоминанию
`python3?` в той же строке, тело читается как проза наравне с отдельным
`.py` — комментарии и докстринги живые, прочие строки гасятся.

Блок, ОБЪЯВИВШИЙ себя синтетикой (`# docnum:synthetic-block` в первых
десяти строках), не читается: положительные контроли грамматики этого же
гейта врут числами по замыслу («12 scenarios» при объявленных 115).
Объявление — комментарий В ГОЛОВЕ блока, а не любое вхождение строки:
константа с тем же текстом лежит в этом же блоке, и признание «по
вхождению» было бы НЕСНИМАЕМЫМ — мутация, убирающая объявление, ничего бы
не меняла, то есть исключение нельзя было бы проверить на зубы.
Самоисключение по имени файла отвергнуто отдельно: оно выключило бы охват
всего конвейера разом.

Зубы этой находки: пин D6 переставлен, добавлены D41 (снять объявление
синтетики — гейт обязан покраснеть на своих же контролях) и D42
(устаревший счёт, подложенный в докстринг блока, обязан краснеть). Прогон
НА МОМЕНТ находки: контроль без мутации ЗЕЛЁНО, мутаций 42, покраснели 42,
перепись якорей 42/42.

Число 42 историей НЕ станет: эти зубы едут в один коммит с зубами #58,
которые добавили D43/D44, поэтому в дереве `EXPECTED_MUTATIONS` идёт
40 → 44, а не 40 → 42. Итоговый прогон на 44 — в записи #58. Урок в общий
счёт: запись волны пинит состояние, которое ПОЕДЕТ, а не то, на котором
находка закрылась; промежуточный счёт называть промежуточным.

### #58 — уровень слоя tweakcc стал дверью

Печатаемое `tweakcc=N` не гейтилось ничем: падение 33 → 14 прошло
вердиктом «красных нет», потому что при отсутствии данных апстрима под
версию правки не пробуются вовсе и поимённых ✗ не появляется ни одного —
для сверки непроходов такая версия выглядит чистой. Предмет соседний с
домом непроходов, но другой: там поимённые ✗, здесь КОЛИЧЕСТВО ✓.

Владелец числа — КОНВЕЙЕР, не свип: свип лишь читает своё поле из лога, а
путь `--update`, которым образ попадает человеку, свипом не проходит вовсе.
Дом объявлений — `tools/tweakcc-expected-applied.txt`. Дверь двусторонняя:
измерено МЕНЬШЕ объявленного — отказ (слой просел молча); измерено БОЛЬШЕ —
тоже отказ (объявление пережило свою причину); строки для версии НЕТ —
отказ с измеренным числом и готовой строкой для вставки. Версия берётся из
БАЙТОВ образа. Слепая ручка `CLAUDE_PATCH_ALLOW_TWEAKCC_FAILURES=1` гасит и
эту дверь, но гашение ОБЪЯВЛЯЕТСЯ в лог. Счёт `✓`-строк идёт под `LC_ALL=C`
— под локалью машины `grep -a -c` на не-ASCII строке считает мимо.

Зубы: случай N зонда пути сборки — 6 сценариев / 7 мутаций над игрушечными
файлами, гоняются САМИ функции конвейера; вклад этой находки поднял объявленные итоги зонда
с 11/10 до 17/17 (все четыре случая K/L/M/N объявляют свой вклад, зонд
отказывает, если сумма расходится с итогом). В коммит едет 23/23: в том же
коммите зонд получил случаи волны #59, и 17/17 — тоже промежуточный счёт.

**Найдено при подготовке коммита, до отправки:** дом уровней был ПУСТ —
одна шапка, ни одной строки версий, — при том что дверь стоит на КАЖДОЙ
сборке безусловно (`claude-patch-all.sh:2989`). То есть состояние было не
«локально зелёным», а обречённым отказать на любой сборке; первый свип на
нём остановлен вручную (exit 143 = TERM), не дожидаясь пяти одинаковых
FATAL'ов. Дом заполнен ИЗМЕРЕННЫМИ числами со ссылкой на прогон-источник в
ЧЕТВЁРТОМ поле. Первая его редакция держала ОДИН суммарный столбец
(250→33, 251→36, 252→36, 257→13, 258→13 — свип каденции 02.09, дерево
fab97b5+dirty; 259→34 — прогон 4 установки 04.09). Столбец снят вместе с
суммарной дверью: сумма смешивает двух владельцев. Урожай 04.09 (снимок
кита с волной 34b) дал по версиям кода/промтов 250→14/19, 251→14/22,
252→14/22, 257→13/20, 258→13/20, 259→13/20 — то есть суммарная дверь
отказала бы на ТРЁХ версиях (257, 258, 259) и прошла бы на 251/252, где
14 рядом с 22 по-прежнему дают объявленные 36. Это довод ЗА разделение, а
не против: сумма молчит ровно до движения корпуса человека, а потом
краснит сборку из-за правки, сделанной вне кита. Грамматика поиска
проверена по всем шести строкам плюс отсутствующая версия (пусто ⇒ отказ
с инструкцией). Урок в общий счёт: **новая дверь с
обязательным домом данных не зелена, пока дом пуст** — «механизм написан»
и «состояние проходит» разные утверждения, и мерить надо то состояние,
которое поедет.

**Прогон случая (n) на дереве, которое поедет, покраснил САМ СТЕНД (04.09).**
Случай (n) писался под первую редакцию двери, а дверь потом переписали в этой
же волне — и стенд остался пришит к ушедшему тексту. Два места: мутация
`N5-version-filter-off` держала якорь `$1==v { … }`, тогда как дверь сверяет
ключ ТРИМЛЕННЫМ (`k==v`) — стенд честно ответил «anchor count=0», то есть зуб
не кусал; и утверждение N11 требовало прежний вид строки о ненайденной
накладке, тогда как дверь теперь называет накладку ПОИМЁННО. Оба починены по
текущему тексту. Заодно закрыты три механизма волны, у которых сценария не
было вовсе: N13 — ключ с отступом принадлежит СВОЕЙ версии (мутация снимает
трим ключа), N14 — две строки на версию отказ (мутация гасит дверь дубля),
N15 — строка ЕСТЬ, поле кода пусто. Последнее — правка самой двери: совет
«впишите строку» в этом состоянии добавил бы вторую строку на версию, то есть
отправил бы человека чинить в отказ по дублю; теперь ветки две, и вторая
называет ровно сломанное. Вклад случая (n) 12/13 → 15/16, объявленные итоги
зонда 23/23 → 26/26 в трёх домах (константы зонда, проза README, строки
D43/D44 таблицы гейта чисел). Урок в общий счёт: **стенд, написанный по
первой редакции механизма, обязан быть перепрогнан ПОСЛЕ переписывания этого
механизма** — «стенд есть» и «стенд кусает сегодняшний текст» разные
утверждения, и разошлись они внутри одной волны, за один день.

### #59 — замок конвейера утекал по наследству

`bash` не ставит close-on-exec на перенаправление, поэтому инструмент кита,
запущенный при открытом замке, отдаёт СВОЕМУ фоновому ребёнку копию
дескриптора. Осиротевший внук держит замок за прогон, который давно
кончился: два свипа отказали на «занятом» замке при полном отсутствии
живой сборки, а `lsof` назвал держателями заглушку `cp` и `sleep`.

Правка полная, а не по месту падения: `N>&-` у КАЖДОГО вызова инструмента
кита под открытым замком. Волна закрыла 14 ЖИВЫХ вызовов в
конвейере, 6 в стенде корпусных инструментов, 2 в зонде замка, 1 в
`sweep.sh` (`8>&-`) и 1 в `fetch-corpus.sh` (`6>&-`); живых закрытий стало
15 в конвейере и 33 в стенде корпусных инструментов. Сырой `grep` по
`N>&-` отвечает 17 и 35 -- он считает и строки комментариев, и в зондах
строки ожиданий их собственных случаев; первая редакция этой записи взяла
в скобки именно его ответ и назвала его «всего». Отказ на занятом замке
теперь САМ называет держателей (`lsof` + `ps` с временем старта), вместо
совета человеку сходить за `lsof` руками.

Дисциплина, державшаяся на памяти автора у каждого нового вызова, заменена
гейтом формы `tools/lockfd-check.py`: после `exec N>` каждый последующий
запуск инструмента кита обязан закрыть дескриптор. Грамматика (логические
строки, тела heredoc как данные, разрезание по операторам оболочки,
подпись запуска = интерпретатор плюс путь кита) проверяется собственным
`--self-check` на синтетике с известными ответами — 21 случай. Оба режима
стоят стадией конвейера с разбором кода: 1 — отказ по существу, 2 — прибор
смотрит не туда, 5 — открытий замка не найдено. Объявленные слепые классы:
запуск через переменную-путь; инлайн-код (`-c`/`-e`/`-E`/`-m`/`-p`/
`--eval`/`--print`) — путь в такой форме ДАННЫЕ, а не исполняемый файл;
`perl` ЦЕЛИКОМ, в любой форме, а не только со встроенным кодом (его
слитные ключи `-0ne`, `-0pi -e` грамматикой ключей не разобраны, а в ките
он встречается только со встроенным кодом — сегмент пропускается, а не
судится наугад); функции, определённые до открытия и вызванные после.
Докстринг прибора первой редакции называл лишь `-c`/`-e` и молчал про
`perl`, хотя код слеп к нему целиком — расхождение текста с кодом снято
правкой ТЕКСТА, потому что слепота к perl здесь измеренно верна. Зонд замка получил сценарий ВНУКА с
отрицательным контролем: тот же сценарий без закрытия обязан держать замок
— зелёная нога без контроля неотличима от сценария, который механизм
никогда не воспроизводил.

Две мелочи того же отказа, найденные критиком волны и починенные тут же.
Шапка «Держатели замка сейчас:» печаталась БЕЗУСЛОВНО, и при пустом `lsof`
(его нет в PATH либо держатель ушёл между попыткой и опросом) человек
получал пустой список и следом совет «завершить названного» — названного
ни одной строкой. Теперь ветки две, и вторая честно говорит, что назвать
некого, не обещая имени. Второе: `lsof -t | while read -r __pid` терял
последнюю строку без перевода — форма `read ... || [[ -n $__pid ]]`; там же
ушедший между `lsof` и `ps` держатель называется голым pid, а не
проглатывается, потому что «ушёл сейчас» и «никого не было» — разные
диагнозы. Сценарий 12 зонда замка пинит обе ветки: зонд задаёт тот же
вопрос, что и конвейер (есть ли у файла живые писатели), и требует
СОВПАДАЮЩУЮ ветку, а вторым прогоном ослепляет `lsof` заглушкой — отказ
обязан остаться отказом (код 3) и потерять совет про названного.

Прогоны формы на этом дереве (04.09, после того как все три волны легли в
рабочее дерево): `lockfd-check --self-check` — 21/21 случаев грамматики;
`lockfd-check` по дереву — зелено; `lock-probe.sh` — все сценарии, включая
новый 12-й, обе его ветки; перепись якорей гейта чисел — 44/44 (не 42:
зубы #58 добавили D43/D44); полный прогон `docnum-bench.py` — мутаций 44,
покраснели 44, код 0. Числа названы ТЕКУЩИЕ, а не те, что стояли в момент
находки: запись волны пинит состояние, которое ещё поедет, и промежуточный
счёт в ней читался бы как история.

## Задача #53 — память цен против меню `/model`: обе половины формулировки опровергнуты, дефект оказался другой (04.09)

Задача была записана как «развести память цен и меню `/model`; снять мёртвый
namespace `cc/*`». Разведка перед реализацией опровергла ОБЕ половины и нашла
третью вещь, которая была действующей прямо в момент чтения.

**Половина первая — разводить нечего, связи нет.** Меню `/model` населяет
патч #2 (discovery): он снимает требование токена внутри
`fetchGatewayModelOptions`, и список приходит из `/v1/models` прокси — сегодня
12 живых. Память цен читается ТОЧЕЧНО: чанк-локальный
`function Pd(e,n){return Object.hasOwn(e,n)?e[n]:void 0}` (образ 2.1.259,
смещение 168802602). Все четыре ценовых сайта (`YVt`, `Kie`, `iee`) делают
`Pd(таблица, id)` — ни один не ПЕРЕЧИСЛЯЕТ ключи. 447 строк цен для меню
невидимы по построению.

**Половина вторая — namespace `cc/*` жив.** Прокси отдаёт
`cc/meta/muse-spark-1.3-contributor` (модель боевого реестра classes
critique/audit/analysis), каталог знает её цену 0.1/0.2 и окно 972K. Снятие
namespace целиком стёрло бы цену ЖИВОЙ модели. Само накопление тоже не про
`cc/*`: из 442 строк 422 держались ТОЛЬКО самоподпиткой (roster включает
собственные ключи конфига), `cc/*` среди них 54; рост объявлен в самом
инструменте как осознанный, с `--prune` как заявленным выходом.

**Что оказалось настоящим дефектом.** Синхрон цен привязан к МОМЕНТУ ПАТЧА —
конвейер так и пишет: «Patching is the one moment this install is already
being touched». Но состав прокси меняется чаще, чем ставятся сборки. Замер:
синхрон писал 04.09 00:35 (бэкап `.backup.20260904-003515`), каталог прокси
обновился 09:46. Девять часов две ЖИВЫЕ модели —
`cc/meta/muse-spark-1.3-contributor` и `omen-alpha` — не имели ни строки цены,
ни строки окна, то есть считались по запасной цене $5/$25 и сжимались на
дефолтном окне вместо 972K и 424K. Для muse-spark это завышение счёта в 50
раз по входу и в 125 раз по выходу. Конвейер описывает ровно этот отказ в
собственном комментарии («That is exactly how glm-5.3 sat unpriced with the
wrong window for a day, 2026-08-15») — и его же мера, «синхронить при патче»,
этот отказ НЕ закрывает: между установками нет ничего, что бы заметило смену
состава.

Симптом снят прогоном `set-model-costs.py` 04.09 10:06 (бэкап
`.backup.20260904-100630`): стало 447 цен и 527 окон, контроль — живых у
прокси 12, без строки цены ИЛИ окна ноль. Свип цены не трогает и вторым
писателем конфига не становится: он объявленно ставит
`CLAUDE_PATCH_SKIP_MODELS=1` во ВСЕХ сборках прогона (`tools/sweep.sh:818`).

**Корень остаётся открытым, и полный фикс — две двери.** (а) У дрейфа нет
детектора: он невидим, пока кто-то не откроет `/cost`. Нужен сценарий в
`tools/costs-bench.py` — живая модель прокси без строки цены или окна в
конфиге краснит стенд, с мутацией равной длины, которая его краснит. (б)
Синхрон нужен триггер вне установки — по смене файла каталога, который пишет
сам VibeProxy, а не только по факту патча.

**Две ловушки прибора, обе поймали меня на этом же предмете.** Первая: счёт
моделей каталога по длине верхнего уровня даёт 13 — это ПРОВАЙДЕРЫ, модели
вложены под `provider.models`, их 553. Первый мой замер объявил «13 моделей в
каталоге» и был неверен; правильный счёт пришёл из отчёта самого инструмента
(565 на тот момент). Вторая — уже записанная в память, но сработавшая снова:
минифицированные имена ЛОКАЛЬНЫ ДЛЯ ЧАНКА. `function Pd(` встречается в образе
семь раз; первое вхождение (смещение 81034425) — реактовский планировщик
полос, и прочитав его, я едва не объявил у поиска цен суффиксное сопоставление
и «столкновение ключей». Ценовой `Pd` лежит на 168802602, в 2569 байтах от
своего потребителя. Правило: у минифицированного имени брать вхождение,
БЛИЖАЙШЕЕ к сайту-потребителю, и проверять, что тело подходит по смыслу.
