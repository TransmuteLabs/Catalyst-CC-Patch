...
| `tools/backup-divergence-probe.sh` | the truth table of the guard that refuses when the named target and tweakcc's backup are different images of the SAME version, and when config carries the build-path probe marker — either borrowed by a probe running now or left behind after SIGKILL; tweakcc would otherwise refresh its backup from the target and rewrite the human's restore point; extracts the real guard and refuses if its anchor is gone. The backup-divergence probe has 12 cases and 1 mutation, which removes the marker branch and must redden its own case — run by `build-path-probe.sh` |
