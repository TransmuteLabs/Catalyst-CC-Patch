...
| `CLAUDE_PATCH_ALLOW_TWEAKCC_FAILURES` | unset | escape hatch for the WHOLE tweakcc layer, not for one door: a crash of the stage stops being fatal; the reconciliation of unapplied patches prints its list instead of refusing; the «no trace of tweakcc in the target» door falls silent; and THREE refusals of the layer are extinguished at once — the level door (the code layer's ATTEMPTS against the declared count, the overlays against the floor in the tweakcc home), the door of the SET of edits switched off by that home's config, and the sentinel of the output's FORM, which otherwise refuses on patch lines standing under no known section. Each extinguishing is ANNOUNCED with a NOTE naming its own door — wave 39c made the code keep that promise: the level door's early return stood ABOVE the sentinel and the two doors of outcome, so THEY were extinguished in silence, and only two NOTEs were ever printed. The level door's NOTE names three measured numbers (attempts, landed, overlays), the set door's names how many edits are switched off, and the sentinel of the FORM, the door of unapplied OVERLAYS and the door of edits that ran and changed NOTHING each print their own when they had something to say. With it set the layer is UNVERIFIED — which is why `tools/sweep.sh` and `tools/build-path-probe.sh` scrub it from the environment of every run they judge, and build-path-probe's case (n) pins both extinguished-door NOTEs (mutations `N6-blind-knob-silent` and `N22-off-blind-silent`) as well as the sentinel's own two (`N24`, `N25`) |
