...
installed if any one of the pipeline's 116 checks fails, and the switch to a new build
...
probe bench, any of the pipeline's 116 checks) leaves the live installation
...
| `tweakcc-patch.js` | the patches themselves, as a script for `tweakcc adhoc-patch`; the 116 checks that verify them live in `claude-patch-all.sh` |
...
## What the patches do (116 pipeline checks)
...
narrowing is deliberate and declared: `set-model-costs.py` never exits fatally —
a failed cost sync is a WARNING, because the image is already built and correct.
...
| `CLAUDE_PROBES_DIR` | `~/.claude/probes` | the probes home: settings, prompts and records of the judge and the watcher |
