...
| `tools/tweakcc-expected-applied.txt` | the declared LEVEL of the tweakcc CODE layer per version, one TAB-separated row per `version<TAB>code-layer attempts<TAB>where the number came from`. THREE fields, not four: wave 39b moved the overlay floor to its own home (`catalyst-prompt-floor.txt`, below), because the two numbers had different owners and only one of them belongs to this repository. Column two counts ATTEMPTS of the code layer (a tick, a cross and a circle alike), so its owner is the roster of our tweakcc fork alone and the row means the same thing on every machine; it is gated TWO-SIDED, since a drop and a rise equally mean the declaration has parted from the roster. The number is written from what the pipeline PRINTS, never from memory; where a row's number is the fork's roster rather than a run of that version, the row says so and the file's header names which versions a run has confirmed. Teeth: case (n) of `tools/build-path-probe.sh` |
...
| `CLAUDE_PATCH_ALLOW_TWEAKCC_FAILURES` | unset | escape hatch for the WHOLE tweakcc layer, not for one door: a crash of the stage stops being fatal; the reconciliation of unapplied patches prints its list instead of refusing; the «no trace of tweakcc in the target» door falls silent; and THREE refusals of the layer are extinguished at once — the level door (the code layer's ATTEMPTS against the declared count, the overlays against the floor in the tweakcc home), the door of the SET of edits switched off by that home's config, and the sentinel of the output's FORM, which otherwise refuses on patch lines standing under no known section. Each extinguishing is ANNOUNCED with a NOTE naming its own door: the level door's names three measured numbers (attempts, landed, overlays), the set door's names how many edits are switched off. With it set the layer is UNVERIFIED — which is why `tools/sweep.sh` and `tools/build-path-probe.sh` scrub it from the environment of every run they judge, and build-path-probe's case (n) pins both extinguished-door NOTEs (mutations `N6-blind-knob-silent` and `N22-off-blind-silent`) as well as the sentinel's own two (`N24`, `N25`) |
