...
installed if any one of the pipeline's 118 checks fails, and the switch to a new build
...
probe bench, any of the pipeline's 118 checks) leaves the live installation
...
| `tweakcc-patch.js` | the patches themselves, as a script for `tweakcc adhoc-patch`; the 118 checks that verify them live in `claude-patch-all.sh` |
...
## What the patches do (118 pipeline checks)
